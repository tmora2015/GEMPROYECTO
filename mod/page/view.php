...
if(empty($existingrate) && !empty($existingcontent)){
	...
	echo "<br>";
	echo "<br>";
	$mform = new rating($baseurl);//muestra estos datos

if ($fromform = $mform->get_data()){
    $rate = $fromform->rate;
    
    $records->courseid = $course->id; //guarda los datos en base de datos
	...
	$mform->display();//muestra formulario
}
}
else if($existingrate){
	echo "Votes:".'&nbsp'.$numberofvotes.".".'&nbsp'."Average".'&nbsp'."($meanofvotes)"; //si existen votos ya por el usuario
	echo "<br>";
	echo "<br>";
	echo '<div class="alert alert-info">You have already voted for this video. Thanks.</div>';
	
	if(empty($existcom)){ //si no existe comentario
